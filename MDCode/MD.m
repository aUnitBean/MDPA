...
global nAtoms0 nAtoms1 nAtoms2 T T0 T1 T2 MarkerSize %nAtomsi is the number of atoms of each type
global doPlotImage PlotCount map im PlotSize ScaleV ScaleF


%assign values/ initialize
...
Masses= zeros(1, 118);   %Assigns all masses in periodic table and stores in array
for i = 1:118
    Masses(i)=i*C.am;
end
...
nAtoms0 = sum(Pty0in); %how many atoms of each kind are there?
nAtoms1 = sum(Pty1in);
nAtoms2 = sum(Pty2in);
